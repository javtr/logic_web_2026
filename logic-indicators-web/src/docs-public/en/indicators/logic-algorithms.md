...
- **Pre-built signal engines** for the most common order flow setups
- **Real-time visual + audio alerts** so you never miss a setup
- **Customizable thresholds** for each algorithm
- **Multi-timeframe analysis** built in
- **Historical backtest view** to see how the algorithms would have performed

## Who is it for?

Traders who want the heavy lifting of pattern detection handled automatically and prefer to receive actionable signals instead of interpreting raw order flow data manually.

## Access the complete documentation

[Sign in to access the technical manual →](/login?next=/dashboard/docs/indicators/logic-algorithms)
